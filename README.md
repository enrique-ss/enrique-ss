# Olá! Eu sou o Enrique Silveira 👋

Estudante de **Licenciatura em Computação (IFSul)** e **Análise e Desenvolvimento de Sistemas (UniFacitie)**. Acredito que conhecimento só faz sentido quando compartilhado e que a tecnologia deve ser acessível a todos. Meu propósito é estar onde meus alunos estão e criar pontes para o conhecimento.

## 🌱 Aprendendo agora

- **Backend:** JavaScript, TypeScript e MySQL com Knex
- **Frontend:** Three.js, Gsap e React
- **Boas práticas:** Clean Code, SOLID e Arquitetura de Dados

## 📂 Projeto em Destaque

### [Biblioteca-Backend-Frontend](https://github.com/enrique-ss/biblioteca-backend-frontend)

- **Stack:** TypeScript, Node.js, MySQL, Express
- **Destaque:** CLI sem regras de negócios, apenas para visualizar requisições backend
- **Aprendizados:** Arquitetura de software, documentar regras de negócio e mesclar backend com frontend
